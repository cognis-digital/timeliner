...
How **timeliner** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `timeliner` lives in the **Host / artifact forensics** cluster.
...
| from | relation | to |
|---|---|---|
| `timeliner` | JSON findings compose with | [`memtriage`](https://github.com/cognis-digital/memtriage), [`evtxhunt`](https://github.com/cognis-digital/evtxhunt), [`browserforensics`](https://github.com/cognis-digital/browserforensics) |
| `timeliner` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `timeliner` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `timeliner` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the host / artifact forensics cluster.** `timeliner` output feeds the next tool:
```bash
timeliner ... --format json > out.json          # this tool's findings (see `timeliner --help`)
...
timeliner ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
timeliner ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
timeliner ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `timeliner`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `timeliner` slots
