"""Native cognis-connect emit for timeliner — forward findings to any platform.

Maps timeliner's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    timeliner ... --format json | timeliner-emit --to stix
    timeliner-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['tags'] = rec.get('tags', []).split(',') if rec.get('tags') else []
